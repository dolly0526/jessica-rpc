...
import com.github.dolly0526.simplerpc.core.nameservice.Metadata;
...
public class MetadataSerializer implements Serializer<Metadata> {
    ...
    public void serialize(Metadata entry, byte[] bytes, int offset, int length) {
        ...
        entry.forEach((k, v) -> {
            byte[] keyBytes = k.getBytes(SimpleRpcConst.DEFAULT_CHARSET);
            buffer.putShort(toShortSafely(keyBytes.length));
            buffer.put(keyBytes);

            buffer.putShort(toShortSafely(v.size()));
            for (URI uri : v) {
                byte[] uriBytes = uri.toASCIIString().getBytes(SimpleRpcConst.DEFAULT_CHARSET);
    ...
    private int entrySize(Map.Entry<String, List<URI>> e) {
        ...
        return Short.BYTES +       // Key string length:               2 bytes
                ...
                e.getValue().stream() // Value list
                        .mapToInt(uri -> {
                            return Short.BYTES +       // Key string length:               2 bytes
                                    uri.toASCIIString().getBytes(SimpleRpcConst.DEFAULT_CHARSET).length;    // Serialized key bytes:   variable length
                        }).sum();
    }
    ...
    public Metadata parse(byte[] bytes, int offset, int length) {
        ...
        for (int i = 0; i < sizeOfMap; i++) {
            ...
            buffer.get(keyBytes);
            String key = new String(keyBytes, SimpleRpcConst.DEFAULT_CHARSET);
            ...
            for (int j = 0; j < uriListSize; j++) {
                ...
                buffer.get(uriBytes);
                URI uri = URI.create(new String(uriBytes, SimpleRpcConst.DEFAULT_CHARSET));
                uriList.add(uri);
            }
